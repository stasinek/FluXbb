...
$lang_admin_options = array(
    'Bad HTTP Referer message'         => 'Плохой HTTP_REFERER. Если вы переместили форум в другую папку или на другой домен, необходимо поправить Базовый URL вручную в базе данных (см. параметр o_base_url в таблице config) после чего очистите кеш, удалив все .php файлы в папке /cache.',
...
// Essentials section
    ...
    'Base URL problem'                 => 'Ваш сервер не поддерживает автоматическое преобразование международных доменных имён. Из-за того, что базовый URL форума содержит спецсимволы, вы <strong>должны</strong> использовать сторонний онлайн-перекодировщик для избежания ошибок типа «Неверный Referer».',
...
// Display Section
    ...
    'Info in posts help'               => 'Показывать информацию об авторе сообщения на странице с темой. Сюда входят данные о месте жительства, дата регистрации, счетчик сообщений и контакты (email и веб-страница).',
    ...
    'Topics per page help'             => 'Значение по умолчанию: количество тем на странице раздела. Пользователь может установить своё значение.',
    'Posts per page label'             => 'Сообщений на страницу',
    'Posts per page help'              => 'Значение по умолчанию: количество сообщений на странице темы. Пользователь может установить своё значение.',
    'Indent label'                     => 'Размер отступа',
    'Indent help'                      => 'Если поставить 8, отступы внутри тегов [code][/code] будут делаться табуляциями. Иначе отступы будут отбиваться пробелами.',
...
// Features section
    ...
    'Quick jump help'                  => 'Разрешить быстрый переход в другой раздел (выпадающий список разделов).',
    ...
    'Search all help'                  => 'Если выключено, поиск будет разрешен только в рамках одного раздела. Выключите, если поисковые запросы ведут к высокой загрузке сервера.',
...
// Reports section
    ...
    'Reporting method label'           => 'Подача жалобы',
    'Internal'                         => 'Админка',
    'By e-mail'                        => 'Email',
    'Both'                             => 'И то и другое',
    'Reporting method help'            => 'Выберите способ манипулирования пользовательскими жалобами. Варианты: либо отображать сообщения в специальном разделе админки, либо отправлять админам на почту (список рассылки есть ниже), либо и то, и другое.',
    'Mailing list label'               => 'Список рассылки',
    'Mailing list help'                => 'Список адресов через запятую. Люди из этого списка будут получать жалобы по email.',
...
// E-mail section
    ...
    'Forum subscriptions label'        => 'Подписки на раздел',
    'Forum subscriptions help'         => 'Разрешить пользователям подписываться на разделы (получать email, если кто-то создал тему).',
    ...
    'SMTP address help'                => 'Адрес внешнего SMTP-сервера, через который отправлять письма. Можно дополнить номером порта, если SMTP не использует порт по умолчанию 25 (пример: mail.myhost.com:3580). Оставьте пустым, чтобы почтой занималась локальная программа mail.',
