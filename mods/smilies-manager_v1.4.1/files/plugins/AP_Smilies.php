...
if (isset($_POST['reord']))
{
	...
	$smilies_img = array_map('trim', $_POST['smilies_img']);
...
elseif (isset($_POST['remove']))
{
	...
	// Delete smilies
	$db->query('DELETE FROM '.$db->prefix.'smilies WHERE id IN ('.implode(', ', array_keys($rem_smilies)).')') or error('Unable to delete smiley', __FILE__, __LINE__, $db->error());
...
?>
										...
										<td><input type="text" name="smilies_code[<?php echo $db_smilies['id'] ?>]" value="<?php echo pun_htmlspecialchars($db_smilies['text']) ?>" size="5" maxlength="60" /></td>
